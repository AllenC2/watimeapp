...
function isRejectedChannelMediaPath(directPath) {
  ...
  return path.startsWith('/o1/') || path.startsWith('/m1/');
}
...
export async function sendChannelImage(sock, jid, imagePath, caption) {
  ...
  try {
    const uploaded = sock.waUploadToServer
      ? await sock.waUploadToServer(filePath, {
          mediaType: 'image',
          fileEncSha256B64: fileSha256.toString('base64'),
          newsletter: true,
        })
      : await createNewsletterUpload(sock)(filePath, {
          mediaType: 'image',
          fileEncSha256B64: fileSha256.toString('base64'),
        });
