...
function summarizeUpload(result, extra = {}) {
  if (!result || typeof result !== 'object') {
    return { empty: true, ...extra };
  }
  const directPath = result.directPath || result.direct_path || null;
  return {
    ...extra,
    status: result.statusCode || extra.status || null,
    keys: Object.keys(result),
    directPath,
    hasUrl: Boolean(result.url || result.mediaUrl),
    handle: result.handle || null,
    hasThumb: Boolean(result.thumbnail_info || result.thumbnailDirectPath),
    thumbPath: result.thumbnail_info?.thumbnail_direct_path || result.thumbnailDirectPath || null,
  };
}
    ...
    const req = https.request(
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          console.log('[WhatsApp] HTTP newsletter-image', {
...
export function createNewsletterUpload(sock) {
  return async (filePath, opts = {}) => {
    ...
    const token = encodeBase64EncodedStringForUpload(String(hash || ''));
    ...
    for (const host of hosts) {
      ...
      try {
        ...
        if (result?.url || result?.direct_path) {
          console.log('[WhatsApp] Media de canal subida', summarizeUpload(result, { mediaType, host: hostname }));
          return {
            mediaUrl: result.url,
...
export async function sendChannelImage(sock, jid, imagePath, caption) {
  ...
  try {
    ...
    if (sock.waUploadToServer) {
      try {
        nativeUploaded = await sock.waUploadToServer(filePath, {
          mediaType: 'image',
          fileEncSha256B64: fileSha256.toString('base64'),
          newsletter: true,
        });
        console.log('[WhatsApp] Resultado subida nativa', summarizeUpload(nativeUploaded));
      } catch (err) {
        console.warn('[WhatsApp] Error subida nativa:', err.message || err);
      }
    }
    ...
    console.log('[WhatsApp] Resultado subida newsletter-image', summarizeUpload(uploaded), {
      nativePath: nativeUploaded?.directPath || null,
    });

    if (!uploaded?.directPath || isRejectedChannelMediaPath(uploaded.directPath)) {
      throw new Error(
        `WhatsApp devolvió una ruta de imagen inválida para el canal (${uploaded?.directPath || 'vacía'}).`
      );
    }
