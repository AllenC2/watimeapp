...
export async function sendChannelImage(sock, jid, imagePath, caption) {
  ...
  try {
    let uploaded = await createNewsletterUpload(sock)(filePath, {
      mediaType: 'image',
      fileEncSha256B64: fileSha256.toString('base64'),
    });

    if (
      (!uploaded?.directPath || isRejectedChannelMediaPath(uploaded.directPath)) &&
      sock.waUploadToServer
    ) {
      console.warn('[WhatsApp] Reintentando subida de canal con el uploader nativo');
      uploaded = await sock.waUploadToServer(filePath, {
        mediaType: 'image',
        fileEncSha256B64: fileSha256.toString('base64'),
        newsletter: true,
      });
