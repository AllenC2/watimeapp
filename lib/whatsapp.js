...
import { runQuery, getQuery, getOne } from './db';
...
import { normalizeIdentifier } from './contacts';
import { authDirFor, listRestorableUserIds, statusPathFor } from './legacy-migrate';
import {
...
async function sendScheduledMessage(sock, msg) {
  let jid = normalizeIdentifier(msg.recipient).identifier;
  let resultJid = jid;
  ...
  if (imagePath && isNewsletterJid(resultJid)) {
    options.upload = createNewsletterUpload(sock);
  }
  const sent = await sock.sendMessage(resultJid, payload, options);
  if (!sent?.key?.id) {
...
function startScheduler(userId) {
  ...
  const tick = async () => {
    ...
    try {
      const userRow = await getOne('SELECT timezone FROM users WHERE id = ?', [userId]);
      const messages = await getQuery(
        `SELECT * FROM scheduled_messages WHERE user_id = ? AND status = 'pending'`,
        [userId]
      );
      const due = messages.filter((msg) =>
        isScheduledDue(msg.scheduled_for, new Date(), userRow?.timezone)
      );
...
export async function restoreWhatsAppSessions({ delayMs = 750, connect = connectWhatsApp } = {}) {
  const ids = listRestorableUserIds();
  console.log(`[WhatsApp] Restaurando ${ids.length} sesión(es)`);
  for (const userId of ids) {
    try {
      await connect(userId);
    } catch (err) {
      console.error(`[WhatsApp] No se pudo restaurar usuario ${userId}:`, err.message || err);
    }
    if (delayMs > 0) {
